...
/*
 * TOSEMU: the only thing changed in this file, in four places.
 *
 * A bitmap's address arrives in two words of the control array, and EmuTOS
 * reads it back as *(MFDB **)&CONTRL[7]. On a 68000 that reads four bytes,
 * which is the whole of the address. On a sixty four bit host it reads eight,
 * so the source read swallows the destination address as its high half and the
 * pointer that comes out is neither.
 *
 * Reading thirty two bits is what the caller wrote. Every address here fits in
 * that, because the program is linked low on purpose - see emuvdi/README - and
 * because the AES writes these through ULONG_AT, which is thirty two bits
 * wide, so it was already storing them that way.
 *
 * Everything else in this file is EmuTOS's, unchanged, from VERSION_1_4.
 */
#define MFDB_AT(words)  ((MFDB *)(uintptr_t)*(const uint32_t *)&(words))
...
void vdi_vr_trnfm(Vwk * vwk)
{
    ...
    src_mfdb = MFDB_AT(CONTRL[7]);
    dst_mfdb = MFDB_AT(CONTRL[9]);
...
setup_info (struct raster_t *raster, struct blit_frame * info)
{
    ...
    /* Get the pointers to the MFDBs */
    src = MFDB_AT(CONTRL[7]);   /* a5, source MFDB */
    dst = MFDB_AT(CONTRL[9]);   /* a4, destination MFDB */

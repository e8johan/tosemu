...
/*
 * TOSEMU: the one thing changed in this file.
 *
 * A G_USERDEF object draws itself: the application hands the AES a routine and
 * the AES calls it. EmuTOS does that with a jsr, which works because the
 * application and the AES are the same kind of code. Here they are not. The
 * routine is 68000 code in the emulated machine and this is host code, so
 * calling it means handing it to the emulator to run, which is what
 * host_call_userdef does.
 *
 * Everything else in this file is EmuTOS's, unchanged, from VERSION_1_4.
 */
static WORD call_usercode(USERBLK *ub, PARMBLK *pb)
{
    return host_call_userdef(ub, pb);
}

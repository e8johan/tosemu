...
/* TOSEMU: where host_menu_begin and host_menu_end are declared */
#include "aeskernel.h"
...
static WORD menu_down(OBJECT *tree, WORD ititle)
{
    ...
    if (do_chg(tree, ititle, SELECTED, TRUE, TRUE, TRUE))
    {
        ...
        /* TOSEMU: and now that there is something to show, somewhere to show
         * it. See menu_sr above. */
        {
            GRECT   t;

            /*
             * The rectangle the menu was drawn in, which is not the one
             * menu_sr saves: that one is padded on every side so that whatever
             * the drawing put just outside the box goes back too. A window has
             * to be the size of what is actually there, and what is actually
             * there is the box and the shadow down its right and bottom.
             */
            ob_actxywh(tree, imenu, &t);
            host_menu_begin(t.g_x, t.g_y,
                            t.g_w + MENU_THICKNESS,
                            t.g_h + MENU_THICKNESS);
        }
    }
